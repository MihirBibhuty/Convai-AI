...
//     characterId: SETTINGS['CHARACTER-ID@nancy'],
//     enableAudio: true, // use false for text only.
// });

export default function NancyScene({ setMessages, messages, backgroundNo, dance, jump, punch, kick, special }) {
    ...
    useEffect(() => {
        let len = messages["nancy"].length;
        let lastMessage = messages["nancy"][len - 1]

        if (conv["user"] && conv["machine"]) {
            if (len == 0 || lastMessage["user"] !== conv["user"]) {
                setMessages({ ...messages, nancy: [...messages["nancy"], conv] })
            }
            else {
                let newMessages = messages["nancy"].slice(0, len - 1);
                setMessages({ ...messages, nancy: [...newMessages, conv] })
            }
        }
    }, [conv])
    // ********* *********
    ...
    return (
        <>
            <Canvas shadows camera={{ position: [0, 0, 15], fov: 30 }}>
                ...
                <NancyModel position={[0, -0.5, 5]} scale={24} animationName={isTalking ? "talk" : "idle"} dance={dance} jump={jump} punch={punch} kick={kick} special={special} />
                <ExplorerModel position={[3, -0.5, 3]} scale={2} animationName={isTalking ? "talk" : "idle"} />
                <MariaWorrierModel position={[-3, -0.5, 3]} scale={2} animationName={isTalking ? "talk" : "idle"} />
