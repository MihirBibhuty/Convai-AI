...
//     characterId: SETTINGS['CHARACTER-ID@explorer'],
//     enableAudio: true, // use false for text only.
// });


export default function ExplorerScene({ setMessages, messages, backgroundNo, dance, jump, punch, kick, special }) {
    ...
    useEffect(() => {
        let len = messages["default"].length;
        let lastMessage = messages["default"][len - 1]

        if (conv["user"] && conv["machine"]) {
            if (len == 0 || lastMessage["user"] !== conv["user"]) {
                setMessages({ ...messages, default: [...messages["default"], conv] })
            }
            else {
                let newMessages = messages["default"].slice(0, len - 1);
                setMessages({ ...messages, default: [...newMessages, conv] })
            }
        }
    }, [conv])
    // ********* *********
    ...
    return (
        <>
            <Canvas shadows camera={{ position: [0, 0, 15], fov: 30 }}>
                ...
                <ExplorerModel position={[0, -0.5, 5]} scale={2} animationName={isTalking ? "talk" : "idle"} dance={dance} jump={jump} punch={punch} kick={kick} special={special} />
